...
use crate::kernel::matmul::matmul_gpu;
...
fn swiglu_elementwise(
    ...
    let buf_w_gate = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("gate"),
        contents: bytemuck::cast_slice(gate),
        usage: wgpu::BufferUsages::STORAGE,
    });
    let buf_w_up = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("up"),
    ...
    let module =
        device.create_shader_module(wgpu::include_wgsl!("../shader/swiglu_elementwise.wgsl"));
